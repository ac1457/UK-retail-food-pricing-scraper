...
class Filemanager:
    ...
    products_json_path = f"{root_path}/records.json"
    products_csv_path = f"{root_path}/products.csv"
    settings_ini_path = f"{root_path}/settings.ini"
    logging_ini_path = f"{root_path}/logging.ini"
    logfile_path = f"{root_path}/logfile.log"
    database_folder = f"{root_path}/data"
...
class Config:
    ...
    def get_request_delay():
        ...
        config = Config.read(Filemanager.settings_ini_path)
        try:
            delay_value = config["Scraping"]["request_delay"]
            ...
            elif isinstance(delay_value, (int, float)):
                return delay_value
            else:
